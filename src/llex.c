/*
 ** $Id: llex.c $
 ** Lexical Analyzer See Copyright Notice in lua.h
 */
...
/* ORDER RESERVED */
static const char *const luaX_tokens[] = {"and",       "break",    "do",      "else",   "elseif", "end",   "false",
...
/*
 ** Anchors a string in scanner's table so that it will not be collected until the end of the compilation; by that time
 ** it should be anchored somewhere. It also internalizes long strings, ensuring there is only one copy of each unique
 ** string.
 */
...
void luaX_setinput(lua_State *L, LexState *ls, ZIO *z, TString *source, int firstchar) {
    ...
    ls->lastline = 1;
    ls->source = source;
    /* all three strings here ("_ENV", "break", "global") were fixed, so they cannot be collected */
 ...
 ** will reject ill-formed numerals. Roughly, it accepts the following pattern:
 **
 **   %d(%x|%.|([Ee][+-]?))* | 0[Xx](%x|%.|([Pp][+-]?))*
 **
 ** The only tricky part is to accept [+-] only after a valid exponent mark, to avoid reading '3-4'
 ** or '0xe+1' as a single number.
 **
...
/*
 ** read a sequence '[=*[' or ']=*]', leaving the last bracket. If sequence is well formed, return its number of '='s +
 ** 2; otherwise, return 1 if it is a single bracket (no '='s and no 2nd bracket);
...
/*
 ** When reading a UTF-8 escape sequence, save everything to the buffer for error reporting in case of errors; 'i'
 ** counts the number of saved characters, so that they can be removed if case of success.
 */
