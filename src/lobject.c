/*
 ** $Id: lobject.c $
 ** Some generic functions over Lua objects See Copyright Notice in lua.h
 */
...
/*
 ** Encodes 'p'% as a floating-point byte, represented as (eeeexxxx). The exponent is represented using excess-7.
 ** Mimicking IEEE 754, the representation normalizes the number when possible, assuming an extra 1 before the mantissa
 ** (xxxx) and adding one to the exponent (eeee) to signal that. So, the real value is (1xxxx) * 2^(eeee - 7 - 1) if
...
/*
 ** Computes 'p' times 'x', where 'p' is a floating-point byte. Roughly, we have to multiply 'x' by the mantissa and
 ** then shift accordingly to the exponent.  If the exponent is positive, both the multiplication and the shift increase
 ** 'x', so we have to care only about overflows. For negative exponents, however, multiplying before the shift keeps
 ** more significant bits, as long as the multiplication does not overflow, so we check which order is best.
 */
...
/*
 ** Convert string 's' to a Lua number (put in 'result'). Return NULL on fail or the address of the ending '\0' on
 ** success. ('mode' == 'x') means a hexadecimal numeral.
 */
...
/*
 ** Convert string 's' to a Lua number (put in 'result') handling the current locale. This function accepts both the
 ** current locale or a dot as the radix mark. If the conversion fails, it may mean number has a dot but locale accepts
 ** something else. In that case, the code copies 's'
 ** to a buffer (because 's' is read-only), changes the dot to the current locale radix mark, and tries to convert
 ** again. The variable 'mode' checks for special characters in the string:
 ...
 ** 'LUA_N2SBUFFSZ' must be enough to accommodate both LUA_INTEGER_FMT and LUA_NUMBER_FMT.  For a long long int, this is
 ** 19 digits plus a sign and a final '\0', adding to 21. For a long double, it can go to a sign, the dot, an exponent
 ** letter, an exponent sign, 4 exponent digits, the final
 ** '\0', plus the significant digits, which are approximately the *_DIG attribute.
 */
...
/*
 ** Convert a float to a string, adding it to a buffer. First try with a not too large number of digits, to avoid noise
 ** (for instance, 1.1 going to "1.1000000000000001"). If that lose precision, so that reading the result back gives a
 ** different number, then do the conversion again with extra precision. Moreover, if the numeral looks like an integer
 ** (without a decimal point or an exponent), add ".0" to its end.
 */
...
/*
 ** Size for buffer space used by 'luaO_pushvfstring'. It should be (LUA_IDSIZE + LUA_N2SBUFFSZ) + a minimal space for
 ** basic messages, so that 'luaG_addinfo' can work directly on the static buffer.
 */
#define BUFVFS cast_uint(LUA_IDSIZE + LUA_N2SBUFFSZ + 95)

/*
 ** Buffer used by 'luaO_pushvfstring'. 'err' signals an error while building result (memory error [1] or buffer
 ** overflow [2]).
 */
...
/*
 ** Push final result from 'luaO_pushvfstring'. This function may raise errors explicitly or through memory errors, so
 ** it must run protected.
 */
