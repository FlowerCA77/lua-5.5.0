/*
 ** $Id: ldump.c $
 ** save precompiled Lua chunks See Copyright Notice in lua.h
 */
 ...
 ** 'b' should not be NULL, except for the last call signaling the end of the dump.
 */
...
/*
 ** Signed integers are coded to keep small values small. (Coding -1 as 0xfff...fff would use too many bytes to save a
 ** quite common value.) A non-negative x is coded as 2x; a negative x is coded as -2x - 1.
 ** (0 => 0; -1 => 1; 1 => 2; -2 => 3; 2 => 4; ...)
 */
 ...
 ** size==0 is followed by an index and means "reuse saved string with that index"; index==0 means NULL.
 ** size>=1 is followed by the string contents with real size==size-1 and means that string, which will be saved with
 ** the next available index. The real size does not include the ending '\0'
 ** (which is not dumped), so adding 1 to it cannot overflow a size_t.
 */
static void dumpString(DumpState *D, TString *ts) {
    if (ts == NULL) {
        ...
        if (!tagisempty(tag)) {                     /* string already saved? */
            ...
            luaH_set(D->L, D->h, &key, &value);    /* h[ts] = nstr integer value does not need barrier*/
        }
    }
}
