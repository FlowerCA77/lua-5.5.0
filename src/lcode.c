/*
 ** $Id: lcode.c $
 ** Code generator for Lua See Copyright Notice in lua.h
 */
...
/*
 ** Return the previous instruction of the current code. If there may be a jump target between the current instruction
 ** and the previous one, return an invalid instruction (to avoid wrong optimizations).
 */
...
/*
 ** Create a OP_LOADNIL instruction, but try to optimize: if the previous instruction is also OP_LOADNIL and ranges are
 ** compatible, adjust range of previous instruction instead of emitting a new one. (For instance, 'local a; local b'
 ** will generate a single opcode.)
 */
...
/*
 ** Create a jump instruction and return its position, so its destination can be fixed later (with
 ** 'fixjump').
 */
...
/*
 ** returns current 'pc' and marks it as a jump target (to avoid wrong optimizations with consecutive instructions not
...
/*
 ** Returns the position of the instruction "controlling" a given jump (that is, its condition), or the jump itself if
 ** it is unconditional.
 */
...
/*
 ** Patch destination register for a TESTSET instruction. If instruction in position 'node' is not a TESTSET, return 0
 ** ("fails"). Otherwise, if 'reg' is not 'NO_REG', set it as the destination register. Otherwise, change instruction to
 ** a simple 'TEST' (produces no register value)
 */
...
/*
 ** Traverse a list of tests, patching their destination address and registers: tests producing values jump to 'vtarget'
 ** (and put their values in 'reg'), other tests jump to 'dtarget'.
 */
 ...
 ** (The assert means that we cannot fix a jump to a forward address because we only know addresses once code is
 ** generated.)
 */
...
/*
 ** Save line info for a new instruction. If difference from last line does not fit in a byte, of after that many
 ** instructions, save a new absolute line info; (in that case, the special value
 ** 'ABSLINEINFO'
 ** in 'lineinfo' signals the existence of this absolute information.) Otherwise, store the difference from last line in
 ** 'lineinfo'.
 */
static void savelineinfo(FuncState *fs, Proto *f, int line) {
    ...
    if (abs(linedif) >= LIMLINEDIFF || fs->iwthabs++ >= MAXIWTHABS) {
        luaM_growvector(fs->ls->L, f->abslineinfo, fs->nabslineinfo, f->sizeabslineinfo, AbsLineInfo, INT_MAX, "lines");
        f->abslineinfo[fs->nabslineinfo].pc = pc;
        f->abslineinfo[fs->nabslineinfo++].line = line;
        linedif = ABSLINEINFO; /* signal that there is absolute information */
        fs->iwthabs = 1;       /* restart counter */
    }
    luaM_growvector(fs->ls->L, f->lineinfo, pc, f->sizelineinfo, ls_byte, INT_MAX, "opcodes");
    f->lineinfo[pc] = cast(ls_byte, linedif);
    fs->previousline = line; /* last line saved */
}

/*
 ** Remove line information from the last instruction. If line information for that instruction is absolute, set
 ** 'iwthabs'
 ** above its max to force the new (replacing) instruction to have absolute line info, too.
 */
...
/*
 ** Emit instruction 'i', checking for array sizes and saving also its line information. Return 'i'
 ** position.
 */
int luaK_code(FuncState *fs, Instruction i) {
    Proto *f = fs->f;
    /* put new instruction in code array */
    luaM_growvector(fs->ls->L, f->code, fs->pc, f->sizecode, Instruction, INT_MAX, "opcodes");
    f->code[fs->pc++] = i;
...
/*
 ** Use scanner's table to cache position of constants in constant list and try to reuse constants. Because some values
 ** should not be used as keys (nil cannot be a key, integer keys can collapse with float keys), the caller must provide
 ** a useful 'key' for indexing the cache.
 */
...
/*
 ** Add a float to list of constants and return its index. Floats with integral values need a different key, to avoid
 ** collision with actual integers. To that end, we add to the number its smaller power-of-two fraction that is still
 ** significant in its scale.
 ** (For doubles, the fraction would be 2^-52). This method is not bulletproof: different numbers may generate the same
 ** key (e.g., very large numbers will overflow to 'inf') and for floats larger than 2^53 the result is still an
 ** integer. For those cases, just generate a new entry. At worst, this only wastes an entry with a duplicate.
 */
...
/*
 ** Check whether 'i' can be stored in an 'sC' operand. Equivalent to (0 <= int2sC(i) && int2sC(i)
 ** <= MAXARG_C) but without risk of overflows in the hidden addition inside 'int2sC'.
 */
...
/*
 ** Get the value of 'var' in a register and generate an opcode to check whether that register is nil. 'k' is the index
 ** of the variable name in the list of constants. If its value cannot be encoded in Bx, a 0 will use '?' for the name.
 */
...
/*
 ** Fix an expression to return one result. If expression is not a multi-ret expression (function call or vararg), it
 ** already returns one result, so nothing needs to be done. Function calls become VNONRELOC expressions (as its result
 ** comes fixed in the base register of the call), while vararg expressions become VRELOC (as OP_VARARG puts its results
 ** where it wants).
...
/*
 ** Ensure expression value is in a register, making 'e' a non-relocatable expression.
 ** (Expression still may have jump lists.)
 */
...
/*
 ** Ensures final expression result (which includes results from its jump lists) is in register
 ** 'reg'. If expression has jumps, need to patch these jumps either to its final position or to
 ** "load" instructions (for those tests that do not produce values).
 */
...
int luaK_exp2anyreg(FuncState *fs, expdesc *e) {
    ...
    if (e->k == VNONRELOC) {  /* expression already has a register? */
        ...
        /* else expression has jumps and cannot change its register to hold the jump values, because it is a local
         * variable. Go through to the default case. */
    }
...
/*
 ** Ensures final expression result is in a valid R/K index (that is, it is either in a register or in 'k' with an index
 ** in the range of R/K indices). Returns 1 iff expression is K.
 */
...
void luaK_storevar(FuncState *fs, expdesc *var, expdesc *ex) {
    ...
    case VVARGIND: {
        needvatab(fs->f); /* function will need a vararg table now, assignment is to a regular table*/
    } /* FALLTHROUGH */
    case VINDEXED: {
 ...
 ** is true, code will jump if 'e' is true.) Return jump position. Optimize when 'e' is 'not'
 ** something, inverting the condition and removing the 'not'.
 */
...
static int isKstr(FuncState *fs, expdesc *e) {
    return (e->k == VK && !hasjumps(e) && e->u.info <= MAXINDEXRK && ttisshrstring(&fs->f->k[e->u.info]));
}
...
/*
 ** Create expression 't[k]'. 't' must have its final result already in a register or upvalue. Upvalues can only be
 ** indexed by literal strings. Keys can be literal strings in the constant table or arbitrary values in registers.
 */
...
/*
 ** Return false if folding can raise an error. Bitwise operations need operands convertible to integers; division
 ...
 ** (everything but 'not'). Expression to produce final result will be encoded in 'e'.
 */
 ...
 ** (everything but logical operators 'and'/'or' and comparison operators). Expression to produce final result will be
 ** encoded in 'e1'.
 */
...
/* Try to code a binary operator negating its second operand. For the metamethod, 2nd operand must keep its original
 * value.
 */
...
/*
 ** Code arithmetic operators ('+', '-', ...). If second operand is a constant in the proper range, use variant opcodes
 ** with K operands.
 */
...
/*
 ** Code commutative operators ('+', '*'). If first operand is a numeric constant, change order of operands to try to
 ** use an immediate or K operator.
 */
...
/*
 ** Code bitwise operations; they are all commutative, so the function tries to put an integer constant as the 2nd
 ** operand (a K operand).
 */
...
/*
 ** Create code for '(e1 .. e2)'. For '(e1 .. e2.1 .. e2.2)' (which is '(e1 .. (e2.1 .. e2.2))', because concatenation
 ** is right associative), merge both CONCATs.
 */
...
/*
 ** Change line information associated with current position, by removing previous info and adding it again with new
 ** line.
 */
 ...
 ** 'tostore' is number of values (in registers 'base + 1',...) to add to table (or LUA_MULTRET to add up to stack top).
 */
