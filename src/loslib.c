/*
 ** $Id: loslib.c $
 ** Standard Operating System library See Copyright Notice in lua.h
 */
...
#if !defined(l_gmtime) /* { */
/*
 ** By default, Lua uses gmtime/localtime, except when POSIX is available, where it uses gmtime_r/localtime_r
 */
 ...
 ** { year=%Y, month=%m, day=%d, hour=%H, min=%M, sec=%S, wday=%w+1, yday=%j, isdst=? }
 ** =======================================================
 */

/*
 ** About the overflow check: an overflow cannot occur when time is represented by a lua_Integer, because either
 ** lua_Integer is large enough to represent all int fields or it is not large enough to represent a time that cause a
 ** field to overflow.  However, if times are represented as doubles and lua_Integer is int, then the time
 ** 0x1.e1853b0d184f6p+55 would cause an overflow when adding 1900 to compute the year.
 */
...
static const char *checkoption(lua_State *L, const char *conv, size_t convlen, char *buff) {
