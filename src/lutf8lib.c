/*
 ** $Id: lutf8lib.c $
 ** Standard library for UTF-8 manipulation See Copyright Notice in lua.h
 */
...
/*
 ** Decode one UTF-8 sequence, returning NULL if byte sequence is invalid.  The array 'limits'
 ** stores the minimum value for each sequence length, to check for overlong representations. Its first entry forces an
 ** error for non-ASCII bytes with no continuation bytes (count == 0).
 */
...
/*
 ** utf8len(s [, i [, j [, lax]]]) --> number of characters that start in the range [i,j], or nil +
 ** current position if 's' is not well formed in that interval
 */
...
/*
 ** offset(s, n, [i])  -> indices where n-th character counting from position 'i' starts and ends; 0 means character at
 ** 'i'.
 */
