/*
 ** $Id: lfunc.c $
 ** Auxiliary functions to manipulate prototypes and closures See Copyright Notice in lua.h
 */
...
/*
 ** Call closing method for object 'obj' with error object 'err'. The boolean 'yy' controls whether the call is
 ** yieldable.
...
/*
 ** Prepare and call a closing method. If status is CLOSEKTOP, the call to the closing method will be pushed at the top
 ** of the stack. Otherwise, values can be pushed right after the 'level' of the upvalue being closed, as everything
 ** after that won't be used again.
 */
...
/*
 ** Close all upvalues and to-be-closed variables up to the given stack level. Return restored
 ** 'level'.
 */
...
lu_mem luaF_protosize(Proto *p) {
    lu_mem sz = cast(lu_mem, sizeof(Proto)) + cast_uint(p->sizep) * sizeof(Proto *) +
                cast_uint(p->sizek) * sizeof(TValue) + cast_uint(p->sizelocvars) * sizeof(LocVar) +
