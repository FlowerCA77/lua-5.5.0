/*
 ** $Id: lparser.c $
 ** Lua Parser See Copyright Notice in lua.h
 */
...
/* maximum number of variable declarations per function (must be smaller than 250, due to the bytecode format) */
#define MAXVARS 200
...
/*
 ** Check that next token is 'what' and skip it. In case of error, raise an error that the expected
 ** 'what' should match a 'who'
...
static int new_varkind(LexState *ls, TString *name, lu_byte kind) {
    ...
    Vardesc *var;
    luaM_growvector(
        L, dyd->actvar.arr, dyd->actvar.n + 1, dyd->actvar.size, Vardesc, SHRT_MAX, "variable declarations"
    );
...
/*
 ** Convert 'nvar', a compiler index level, to its corresponding register. For that, search for the highest variable
 ** below that level that is in a register and uses its register index ('ridx') plus one.
 */
...
/*
 ** Raises an error if variable described by 'e' is read only; moreover, if 'e' is t[exp] where t is the vararg
 ** parameter, change it to index a real table. (Virtual vararg tables cannot be changed.)
 */
...
/*
 ** Look for an active variable with the name 'n' in the function 'fs'. If found, initialize 'var'
 ** with it and return its expression kind; otherwise return -1. While searching, var->u.info==-1 means that the
 ** preambular global declaration is active (the default while there is no other global declaration);
 ** var->u.info==-2 means there is no active collective declaration (some previous global declaration but no collective
 ** declaration);
 ** and var->u.info>=0 points to the inner-most (the first one found) collective declaration, if there is one.
 */
...
/*
 ** Find a variable with the given name 'n'. If it is an upvalue, add this upvalue into all intermediate functions. If
 ** it is a global, set
...
static void buildglobal(LexState *ls, TString *varname, expdesc *var) {
    ...
    if (var->k == VGLOBAL) {
        luaK_semerror(ls, "%s is global when accessing variable '%s'", LUA_ENV, getstr(varname));
    }
...
static l_noret jumpscopeerror(LexState *ls, Labeldesc *gt) {
    ...
    luaK_semerror(
        ls, "<goto %s> at line %d jumps into the scope of '%s'", getstr(gt->name), gt->line, varname
    ); /* raise the error */
}

/*
 ** Closes the goto at index 'g' to given 'label' and removes it from the list of pending gotos. If it jumps into the
 ** scope of some variable, raises an error. The goto needs a CLOSE if it jumps out of a block with upvalues, or out of
 ** the scope of some variable and the block has upvalues (signaled by parameter 'bup').
 */
...
/*
 ** Search for an active label with the given name, starting at index 'ilb' (so that it can search for all labels in
 ** current block or all labels in current function).
 */
...
/*
 ** Create an entry for the goto and the code for it. As it is not known at this point whether the goto may need a
 ** CLOSE, the code has a jump followed by an CLOSE. (As the CLOSE comes after the jump, it is a dead instruction; it
 ** works as a placeholder.) When the goto is closed against a label, if it needs a CLOSE, the two instructions swap
 ...
 ** 'last' tells whether label is the last non-op statement in its block. Solves all pending gotos to this new label and
 ** adds a close instruction if necessary. Returns true iff it added a close instruction.
 */
...
/*
 ** Traverse the pending gotos of the finishing block checking whether each match some label of that block. Those that
 ** do not match are
 ** "exported" to the outer block, to be solved there. In particular, its 'nactvar' is updated with the level of the
 ** inner block, as the variables of the inner block are now out of scope.
 */
static void solvegotos(FuncState *fs, BlockCnt *bl) {
    ...
    while (igt < gl->n) {                     /* for each pending goto */
        ...
        if (lb != NULL) {                      /* found a match? */
            ...
            /* block has variables to be closed and goto escapes the scope of some variable? */
...
static l_noret undefgoto(LexState *ls, Labeldesc *gt) {
    ...
    luaK_semerror(ls, "no visible label '%s' for <goto> at line %d", getstr(gt->name), gt->line);
}
...
/*
 ** codes instruction to create new closure in parent function. The OP_CLOSURE instruction uses the last available
 ** register, so that, if it invokes the GC, the GC knows which registers are in use at that time.
 **
 */
 ...
 ** 'until' closes syntactical blocks, but do not close scope, so it is handled in separate.
 */
...
/*
 ** Compute a limit for how many registers a constructor can use before emitting a 'SETLIST'
 ** instruction, based on how many registers are available.
 */
...
/*
 ** check whether, in an assignment to an upvalue/local variable, the upvalue/local variable is begin used in a previous
 ** assignment to a table. If so, save original upvalue/local value in a safe place and use this safe copy in the
 ** previous assignment.
 */
 ...
 **
 ** assignment -> suffixedexp restassign restassign -> ',' suffixedexp restassign | '=' explist
 */
...
static void checkrepeated(LexState *ls, TString *name) {
    ...
    if (l_unlikely(lb != NULL)) {                                                           /*
                                                                                             * already defined?
                                                                                             */
        luaK_semerror(ls, "label '%s' already defined on line %d", getstr(name), lb->line); /* error
                                                                                             */
    }
}
...
/*
 ** Recursively traverse list of globals to be initalized. When going, generate table description for the global. In the
 ** end, after all indices have been generated, read list of initializing expressions. When returning, generate the
 ** assignment of the value on the stack to the corresponding table description. 'n' is the variable being handled,
 ** range [0, nvars - 1].
 */
