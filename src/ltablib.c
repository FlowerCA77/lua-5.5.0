/*
 ** $Id: ltablib.c $
 ** Library for Table Manipulation See Copyright Notice in lua.h
 */
...
/*
 ** Copy elements (1[f], ..., 1[e]) into (tt[t], tt[t+1], ...). Whenever possible, copy in increasing order, which is
 ** better for rehashing.
...
/*
 ** Type for array indices. These indices are always limited by INT_MAX, so it is safe to cast them to lua_Integer even
 ** for Lua 32 bits.
 */
...
/*
 ** Produce a "random" 'unsigned int' to randomize pivot choice. This macro is used only when 'sort'
 ** detects a big imbalance in the result of a partition. (If you don't want/need this "randomness",
 ** ~0 is a good choice.)
 */
 ...
 ** precondition: a[lo] <= P == a[up-1] <= a[up], so it only needs to do the partition from lo + 1 to up - 2.
...
static IdxT partition(lua_State *L, IdxT lo, IdxT up) {
    ...
    for (;;) {
        ...
        /* after the loop, a[i] >= P and a[lo .. i - 1] < P  (a) next loop: repeat --j while P <
         * a[j]*/
