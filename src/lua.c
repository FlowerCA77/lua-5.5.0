/*
 ** $Id: lua.c $
 ** Lua stand-alone interpreter See Copyright Notice in lua.h
 */
...
/*
 ** Function to be called at a C signal. Because a C signal cannot just change a Lua state (as there is no proper
 ** synchronization), this function only sets a hook that, when called, will stop the interpreter.
 */
 ...
 ** index 0, it has 'argv[script]', which is the script name. The arguments to the script (everything after 'script') go
 ** to positive indices;
 ** other arguments (before the script name) go to negative indices. If there is no script name, assume interpreter's
 ** name as base.
 ** (If there is no interpreter's name either, 'script' is -1, so table sizes are zero.)
 */
...
/*
 ** Receives 'globname[=modname]' and runs 'globname = require(modname)'. If there is no explicit modname and globname
 ** contains a '-', cut the suffix after '-' (the "version") to make the global name.
 */
...
/*
 ** Traverses all arguments from 'argv', returning a mask with those needed before running any Lua code or an error code
 ** if it finds any invalid argument. In case of error, 'first' is the index of the bad argument.  Otherwise, 'first' is
 ** -1 if there is no program name, 0 if there is no script name, or the index of the script name.
 */
 ...
 ** 'W', which also affects the state. Returns 0 if some code raises an error.
 */
...
/*
 ** lua_stdin_is_tty detects whether the standard input is a 'tty' (that is, whether we're running lua interactively).
 */
...
static void lua_saveline(const char *line) {
    if (l_addhist != NULL) { /* is there an 'add_history'? */
        (*l_addhist)(line);  /* use it else nothing to be done*/
    }
}

static void lua_freeline(char *line) {
    if (l_readline != NULL) { /* is there a 'readline'? */
        free(line);           /* free line created by it else 'lua_readline' used an automatic buffer;
                               * nothing to free*/
    }
}
...
/*
 ** Return the string to be used as a prompt by the interpreter. Leave the string (or nil, if using the default value)
 ** on the stack, to keep it anchored.
 */
...
/*
 ** Check whether 'status' signals a syntax error and the error message at the top of the stack ends with the above mark
...
/*
 ** Try to compile line on the stack as 'return <line>;'; on return, stack has either compiled chunk or original line
...
/*
 ** Read multiple lines until a complete Lua statement or an error not for an incomplete statement. Start with first
 ** line already read in the stack.
 */
...
/*
 ** Read a line and try to load (compile) it first as an expression (by adding "return " in front of it) and second as a
 ** statement. Return the final status of load/call with the resulting function (if any) in the top of the stack.
 */
