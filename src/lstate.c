...
/*
 ** these macros allow user-specific actions when a thread is created/deleted
 */
...
/*
 ** set GCdebt to a new value keeping the real number of allocated objects (GCtotalobjs - GCdebt) invariant and avoiding
 ** overflows in
 ** 'GCtotalobjs'.
 */
...
/*
 ** Called when 'getCcalls(L)' larger or equal to LUAI_MAXCCALLS. If equal, raises an overflow error. If value is larger
 ** than LUAI_MAXCCALLS (which means it is handling an overflow) but not much larger, does not report an error (to allow
 ** overflow handling to work).
 */
...
LUA_API lua_State *lua_newstate(lua_Alloc f, void *ud, unsigned seed) {
    int i;
    lua_State *L;
    global_State *g = cast(global_State *, (*f)(ud, NULL, LUA_TTHREAD, sizeof(global_State)));
    if (g == NULL) {
        return NULL;
    }
    L = &g->mainth.l;
