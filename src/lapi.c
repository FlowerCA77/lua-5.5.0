/*
 ** $Id: lapi.c $
 ** Lua API See Copyright Notice in lua.h
 */
...
/*
 ** Convert an acceptable index to a pointer to its respective value. Non-valid indices return the special nil value
 ** 'G(L)->nilvalue'.
 */
...
LUA_API void lua_closeslot(lua_State *L, int idx) {
    ...
    api_check(L, (L->ci->callstatus & CIST_TBC) && (L->tbclist.p == level), "no variable to close at given level");
 ...
 ** (auxiliary to 'lua_rotate') Note that we move(copy) only the value inside the stack.
...
/*
 ** Returns a pointer to the internal representation of an object. Note that ISO C does not allow the conversion of a
 ** pointer to function to a 'void*', so the conversion here goes through a
 ** 'size_t'. (As the returned pointer is only informative, this conversion should not be a problem.)
 */
...
/*
 ** The following function assumes that the registry cannot be a weak table; so, an emergency collection while using the
 ** global table cannot collect it.
 */
...
#define checkresults(L, na, nr)                                                                                        \
    (api_check(                                                                                                        \
         L, (nr) == LUA_MULTRET || (L->ci->top.p - L->top.p >= (nr) - (na)),                                           \
         "results from function overflow current stack size"                                                           \
     ),                                                                                                                \
     api_check(L, LUA_MULTRET <= (nr) && (nr) <= MAXRESULTS, "invalid number of results"))
