/*
 ** $Id: ltable.c $
 ** Lua tables (hash) See Copyright Notice in lua.h
 */
...
/*
 ** Implementation of tables (aka arrays, objects, or hash tables). Tables keep its elements in two parts: an array part
 ** and a hash part. Non-negative integer keys are all candidates to be kept in the array part. The actual size of the
 ** array is the largest 'n' such that more than half the slots between 1 and n are in use. Hash uses a mix of chained
 ** scatter table with Brent's variation. A main invariant of these tables is that, if an element is not in its main
 ** position (i.e. the 'original' position that its hash gives to it), then the colliding element is in its own main
 ** position. Hence even when the load factor reaches 100%, performance remains good.
 */
...
/*
 ** Only hash parts with at least 2^LIMFORLAST have a 'lastfree' field that optimizes finding a free slot. That field is
 ** stored just before the array of nodes, in the same block. Smaller tables do a complete search when looking for a
 ** free slot.
 */
#define LIMFORLAST 3 /* log2 of real limit (8) */

/*
 ** The union 'Limbox' stores 'lastfree' and ensures that what follows it is properly aligned to store a Node.
 */
typedef struct {
    Node *dummy;
    Node follows_pNode;
...
/*
 ** MAXABITS is the largest integer such that 2^MAXABITS fits in an unsigned int.
 */
#define MAXABITS (l_numbits(int) - 1)

/*
 ** MAXASIZEB is the maximum number of elements in the array part such that the size of the array fits in 'size_t'.
 */
#define MAXASIZEB (MAX_SIZET / (sizeof(Value) + 1))

/*
 ** MAXASIZE is the maximum size of the array part. It is the minimum between 2^MAXABITS and MAXASIZEB.
 */
#define MAXASIZE (((1u << MAXABITS) < MAXASIZEB) ? (1u << MAXABITS) : cast_uint(MAXASIZEB))

/*
 ** MAXHBITS is the largest integer such that 2^MAXHBITS fits in a signed int.
 */
#define MAXHBITS (MAXABITS - 1)

/*
 ** MAXHSIZE is the maximum size of the hash part. It is the minimum between 2^MAXHBITS and the maximum size such that,
 ** measured in bytes, it fits in a 'size_t'.
 */
...
/*
 ** Common hash part for tables with empty hash parts. That allows all tables to have a hash part, avoiding an extra
 ** check ("is there a hash part?") when indexing. Its sole node has an empty value and a key (DEADKEY, NULL) that is
 ** different from any valid TValue.
 */
...
/*
 ** Hash for integers. To allow a good hash, use the remainder operator ('%'). If integer fits as a non-negative int,
 ** compute an int remainder, which is faster. Otherwise, use an unsigned-integer remainder, which uses all bits and
...
/*
 ** Hash for floating-point numbers. The main computation should be just n = frexp(n, &i); return (n INT_MAX) + i but
 ** there are some numerical subtleties. In a two-complement representation, INT_MAX may not have an exact
 ** representation as a float, but INT_MIN does; because the absolute value of 'frexp' is smaller than 1 (unless 'n' is
 ** inf/NaN), the absolute value of the product
 ** 'frexp * -INT_MIN' is smaller or equal to INT_MAX. Next, the use of 'unsigned int' avoids overflows when adding 'i';
...
/*
 ** Check whether key 'k1' is equal to the key in node 'n2'. This equality is raw, so there are no metamethods. Floats
 ** with integer values have been normalized, so integers cannot be equal to floats. It is assumed that 'eqshrstr' is
 ** simply pointer equality, so that short strings are handled in the default case.  The flag
 ** 'deadok' means to accept dead keys as equal to their original values.
 ** (Only collectable objects can produce dead keys.) Note that dead long strings are also compared by identity.  Once a
 ** key is dead, its corresponding value may be collected, and then another value can be created with the same address.
 ** If this other value is given to 'next', 'equalkey'
 ** will signal a false positive. In a regular traversal, this situation should never happen, as all keys given to
 ** 'next' came from the table itself, and therefore could not have been collected. Outside a regular traversal, we have
 ** garbage in, garbage out. What is relevant is that this false positive does not break anything.  (In particular,
 ** 'next' will return some other valid item on the table or nil.)
 */
...
/*
 ** "Generic" get version. (Not that generic: not valid for integers, which may be in array part, nor for floats with
 ** integral values.) See explanation about 'deadok' in function 'equalkey'.
 */
...
/*
 ** Return the index 'k' if 'k' is an appropriate key to live in the array part of a table, 0 otherwise.
 */
#define arrayindex(k) checkrange(k, MAXASIZE)

/*
 ** Check whether an integer key is in the array part of a table and return its index there, or zero.
 */
...
/*
 ** returns the index of a 'key' for table traversals. First goes all elements in the array part, then elements in the
 ** hash part. The beginning of a traversal is signaled by 0.
 */
 ...
 ** 'nums' is a "count array" where 'nums[i]' is the number of integer keys between 2^(i - 1) + 1 and 2^i. Note that
 ** 'na' is the summation of 'nums'.
 */
typedef struct {
    unsigned total;
    unsigned na;
 ...
 ** hash nodes. (A hash node uses ~3 times more memory than an array entry: Two values plus 'next'
 ** versus one value.) Evaluate with size_t to avoid overflows.
 */
#define arrayXhash(na, nh) (cast_sizet(na) <= cast_sizet(nh) * 3)

/*
 ** Compute the optimal size for the array part of table 't'. This size maximizes the number of elements going to the
 ** array part while satisfying the condition 'arrayXhash' with the use of memory if all those elements went to the hash
 ** part.
 ** 'ct->na' enters with the total number of array indices in the table and leaves with the number of keys that will go
 ** to the array part;
...
static unsigned computesizes(Counters *ct) {
    ...
    /* traverse slices while 'twotoi' does not overflow and total of array indices still can satisfy
     * 'arrayXhash' against the array size */
...
/*
 ** Count keys in hash part of table 't'. As this only happens during a rehash, all nodes have been used. A node can
 ** have a nil value only if it was deleted after being created.
 */
...
/*
 ** Resize the array part of a table. If new size is equal to the old, do nothing. Else, if new size is zero, free the
 ** old array. (It must be present, as the sizes are different.) Otherwise, allocate a new array, move the common
 ** elements to new proper position, and then frees the old array. We could reallocate the array, but we still would
 ** need to move the elements to their new position, so the copy implicit in realloc is a waste. Moreover, most
 ** allocators will move the array anyway when the new size is double the old one (the most common case).
 */
static Value *resizearray(lua_State *L, Table *t, unsigned oldasize, unsigned newasize) {
...
/*
 ** Creates an array for the hash part of a table with the given size, or reuses the dummy node if size is zero. The
 ** computation for size overflow is in two steps: the first comparison ensures that the shift in the second one does
 ** not overflow.
 */
...
/*
 ** Exchange the hash part of 't1' and 't2'. (In 'flags', only the dummy bit must be exchanged: The
 ** 'isrealasize' is not related to the hash part, and the metamethod bits do not change during a resize, so the "real"
 ** table can keep their values.)
 */
...
/*
 ** Resize table 't' for the new given sizes. Both allocations (for the hash part and for the array part) can fail,
 ** which creates some subtleties. If the first allocation, for the hash part, fails, an error is raised and that is it.
 ** Otherwise, it copies the elements from the shrinking part of the array (if it is shrinking) into the new hash. Then
 ** it reallocates the array part. If that fails, the table is in its original state; the function frees the new hash
 ** part and then raises the allocation error. Otherwise, it sets the new hash part into the table, initializes the new
 ** part of the array (if any) with nils and reinserts the elements of the old hash back into the new parts of the
 ** table. Note that if the new size for the array part ('newasize') is equal to the old one ('oldasize'), this function
 ** will do nothing with that part.
 */
...
/*
 ** Rehash a table. First, count its keys. If there are array indices outside the array part, compute the new best size
 ** for that part. Then, resize the table.
 */
static void rehash(lua_State *L, Table *t, const TValue *ek) {
    ...
    if (ct.deleted) { /* table has deleted entries? */
        /* insertion-deletion-insertion: give hash some extra size to avoid repeated resizings */
        nsize += nsize >> 2;
    }
...
/*
 ** Inserts a new key into a hash table; first, check whether key's main position is free. If not, check whether
 ** colliding node is in its main position or not: if it is not, move colliding node to an empty place and put new key
 ** in its main position; otherwise (colliding node is in its main position), new key goes to an empty position. Return
 ** 0 if could not insert key (could not find a free space).
 */
static int insertkey(Table *t, const TValue *key, TValue *value) {
...
lu_byte luaH_get(Table *t, const TValue *key, TValue *res) {
    ...
    case LUA_VNUMFLT: {
        ...
        if (luaV_flttointeger(fltvalue(key), &k, F2Ieq)) { /* integral index? */
            return luaH_getint(t, k, res);                 /* use specialized version else...*/
        }
    } /* FALLTHROUGH */
    default:
 ...
 ** {x=1, y=2}), which creates a key in a table that has no metatable, it is not old/black, and it already has space for
 ** the key.
 */
...
int luaH_psetshortstr(Table *t, TString *key, TValue *val) {
    ...
    /* Else, either table has new-index metamethod, or it needs barrier, or it needs to rehash for the new key. In any
     * of these cases, the operation cannot be completed here. Return a code for the caller. */
    return retpsetcode(t, slot);
}
...
int luaH_pset(Table *t, const TValue *key, TValue *val) {
    ...
    case LUA_VNUMFLT: {
        lua_Integer k;
        if (luaV_flttointeger(fltvalue(key), &k, F2Ieq)) { /* integral index? */
            return psetint(t, k, val);                     /* use specialized version else...*/
        }
    } /* FALLTHROUGH */
    default:
...
/*
 ** Finish a raw "set table" operation, where 'hres' encodes where the value should have been (the result of a previous
 ** 'pset' operation). Beware: when using this function the caller probably need to check a GC barrier and invalidate
 ** the TM cache.
 */
...
/*
 ** Try to find a boundary in the hash part of table 't'. From the caller, we know that 'asize + 1'
 ** is present. We want to find a larger key that is absent from the table, so that we can do a binary search between
 ** the two keys to find a boundary. We keep doubling 'j' until we get an absent index.  If the doubling would overflow,
 ** we try LUA_MAXINTEGER. If it is absent, we are ready for the binary search.
 ** ('j', being max integer, is larger or equal to 'i', but it cannot be equal because it is absent while 'i' is
 ** present.) Otherwise, 'j' is a boundary. ('j + 1' cannot be a present integer key because it is not a valid integer
 ** in Lua.) About 'rnd': If we used a fixed algorithm, a bad actor could fill a table with only the keys that would be
 ** probed, in such a way that a small table could result in a huge length. To avoid that, we use the state's seed as a
 ** source of randomness. For the first probe, we "randomly double" 'i' by adding to it a random number roughly its
 ** width.
 */
...
/*
 ** Try to find a border in table 't'. (A 'border' is an integer index such that t[i] is present and t[i+1] is absent,
 ** or 0 if t[1] is absent, or 'maxinteger' if t[maxinteger] is present.) If there is an array part, try to find a
 ** border there. First try to find it in the vicinity of the previous result (hint), to handle cases like 't[#t + 1] =
 ** val' or 't[#t] = nil', that move the border by one entry. Otherwise, do a binary search to find the border. If there
 ** is no array part, or its last element is non empty, the border may be in the hash part.
 */
lua_Unsigned luaH_getn(lua_State *L, Table *t) {
    ...
    if (asize > 0) { /* is there an array part? */
        ...
        /* last element non empty; set a hint to speed up finding that again (keys in the hash part cannot be hints)*/
        *lenhint(t) = asize;
    }
