...
/*
 ** You can define LUA_FLOORN2I if you want to convert floats to integers by flooring them (instead of raising an error
 ** if they are not integral values)
 */
...
/*
 ** Special case of 'luaV_fastget' for integers, inlining the fast case of 'luaH_getint'.
 */
...
#define luaV_fastset(t, k, val, hres, f) (hres = (!ttistable(t) ? HNOTATABLE : f(hvalue(t), k, val)))
...
LUAI_FUNC lu_byte luaV_finishget(lua_State *L, const TValue *t, TValue *key, StkId val, lu_byte tag);
LUAI_FUNC void luaV_finishset(lua_State *L, const TValue *t, TValue *key, TValue *val, int aux);
