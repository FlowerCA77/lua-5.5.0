/*
 ** $Id: liolib.c $
 ** Standard I/O (and system) library See Copyright Notice in lua.h
 */
...
/*
 ** Change this macro to accept other modes for 'fopen' besides the standard ones.
 */
...
/*
 ** {======================================================
 ** l_popen spawns a new process connected to the current one through the file streams.
 ** =======================================================
 */
...
/*
 ** When creating file handles, always creates a 'closed' file handle before opening the actual file; so, if there is a
 ** memory error, the handle is in a consistent state.
 */
...
/*
 ** Calls the 'close' function from a file handle. The 'volatile' avoids a bug in some versions of the Clang compiler
 ** (e.g., clang 3.0 for 32 bits).
 */
...
/*
 ** maximum number of arguments to 'f:lines'/'io.lines' (it + 3 must fit in the limit for upvalues of a closure)
 */
#define MAXARGLINE 250

/*
 ** Auxiliary function to create the iteration function for 'lines'. The iteration function is a closure over
 ** 'io_readline', with the following upvalues:
 ** 1) The file being read (first value in the stack) 2) the number of arguments to read 3) a boolean, true iff file has
 ** to be closed when finished ('toclose')
...
/*
 ** Return an iteration function for 'io.lines'. If file has to be closed, also returns the file itself as a second
 ** result (to be closed as the state at the exit of a generic for).
 */
...
/*
 ** Read a number: first reads a valid prefix of a numeral into a buffer. Then it calls
 ** 'lua_stringtonumber' to check whether the format is correct and to convert it to a Lua number.
 */
