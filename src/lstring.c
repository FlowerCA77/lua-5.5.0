/*
 ** $Id: lstring.c $
 ** String table (keeps all strings handled by Lua) See Copyright Notice in lua.h
 */
...
/*
 ** Initial size for the string table (must be power of 2). The Lua core alone registers ~50 strings (reserved words +
 ** metaevent keys + a few others). Libraries would typically add a few dozens more.
 */
...
void luaS_resize(lua_State *L, int nsize) {
    ...
    if (l_unlikely(newvect == NULL)) {           /* reallocation failed? */
        if (nsize < osize) {                     /* was it shrinking table? */
            tablerehash(tb->hash, nsize, osize); /* restore to original size leave table as it was*/
        }
...
static TString *internshrstr(lua_State *L, const char *str, size_t l) {
    ...
    for (ts = *list; ts != NULL; ts = ts->u.hnext) {
        if (l == cast_uint(ts->shrlen) && (memcmp(str, getshrstr(ts), l * sizeof(char)) == 0)) {
            /* found! */
...
/*
 ** Create or reuse a zero-terminated string, first checking in the cache (using the string address as a key). The cache
 ** can contain only zero-terminated strings, so it is safe to use 'strcmp' to check hits.
 */
...
TString *luaS_newextlstr(lua_State *L, const char *s, size_t len, lua_Alloc falloc, void *ud) {
    struct NewExt ne;
    if (!falloc) {
        ne.kind = LSTRFIX;
