/*
 ** $Id: lvm.c $
 ** Lua virtual machine See Copyright Notice in lua.h
 */
...
/*
 ** 'l_intfitsf' checks whether a given integer is in the range that can be converted to a float without rounding. Used
 ** in comparisons.
 */
...
/*
 ** Check whether some integers may not fit in a float, testing whether (maxinteger >> NBM) > 0.
 ** (That implies (1 << NBM) <= maxinteger.) (The shifts are done in parts, to avoid shifting by more than the size of
 ** an integer. In a worst case, NBM == 113 for long double and sizeof(long)
 ** == 32.)
 */
...
/*
 ** Try to convert a value from string to a number value. If the value is not a string or is a string not representing a
 ** valid numeral (or if coercions from strings to numbers are disabled via macro 'cvt2num'), do not modify 'result'
 ** and return 0.
 */
...
/*
 ** try to convert a value to an integer, rounding according to 'mode', without string coercion.
 ...
 ** (The following explanation assumes a positive step; it is valid for negative steps mutatis mutandis.) If the limit
 ** is an integer or can be converted to an integer, rounding down, that is the limit. Otherwise, check whether the
 ** limit can be converted to a float. If the float is too large, clip it to LUA_MAXINTEGER.  If the float is too
 ** negative, the loop should not run, because any initial integer value is greater than such limit; so, the function
 ** returns true to signal that. (For this latter case, no integer limit would be correct; even a limit of
 ** LUA_MININTEGER would run the loop once for an initial value equal to LUA_MININTEGER.)
 */
...
/*
 ** Prepare a numerical for loop (opcode OP_FORPREP). Before execution, stack is as follows:
 **   ra     : initial value ra + 1 : limit ra + 2 : step Return true to skip the loop. Otherwise, after preparation,
 ** stack will be as follows:
 **   ra     : loop counter (integer loops) or limit (float loops) ra + 1 : step ra + 2 : control variable
 */
...
/*
 ** Execute a step of a float numerical for loop, returning true iff the loop must continue. (The integer case is
    ...
    for (loop = 0; loop < MAXTAGLOOP; loop++) {
        if (tag == LUA_VNOTABLE) { /* 't' is not a table? */
            ...
            if (l_unlikely(notm(tm))) {
                luaG_typeerror(L, t, "index"); /* no metamethod else will try the metamethod*/
            }
        } else {                                            /* 't' is a table */
        ...
        if (!tagisempty(tag)) {
            return tag; /* done else repeat (tail call 'luaV_finishget')*/
        }
    }
...
/*
 ** Finish a table assignment 't[key] = val'. About anchoring the table before the call to
 ** 'luaH_finishset':
 ** This call may trigger an emergency collection. When loop>0, the table being accessed is a field in some metatable.
 ** If this metatable is weak and the table is not anchored, this collection could collect that table while it is being
 ** updated.
 */
 ...
 ** -greater than zero if 'ts1' is less-equal-greater than 'ts2'. The code is a little tricky because it allows '\0' in
 ** the strings and it uses 'strcoll' (to respect locales) for each segment of the strings. Note that segments can
 ** compare equal but still have different lengths.
 */
...
/*
 ** Check whether integer 'i' is less than float 'f'. If 'i' has an exact representation as a float ('l_intfitsf'),
 ** compare numbers as floats. Otherwise, use the equivalence 'i < f <=> i <
 ** ceil(f)'. If 'ceil(f)' is out of integer range, either 'f' is greater than all integers or less than all integers.
 ** (The test with 'l_intfitsf' is only for performance; the else case is correct for all values, but it is slow due to
 ** the conversion from float to int.) When 'f' is NaN, comparisons must result in false.
 */
...
int luaV_equalobj(lua_State *L, const TValue *t1, const TValue *t2) {
    ...
    if (ttype(t1) != ttype(t2)) { /* not the same type? */
        ...
        case LUA_VNUMINT: { /* integer == float? */
            /* integer and float can only be equal if float has an integer value equal to the integer */
            lua_Integer i2;
        ...
        case LUA_VSHRSTR:
        case LUA_VLNGSTR: {
            /* compare two strings with different variants: they can be equal when one string is a short string and the
             * other is an external string  */
            return luaS_eqstr(tsvalue(t1), tsvalue(t2));
        }
        default:
            /* only numbers (integer/float) and strings (long/short) can have equal values with different variants */
            return 0;
        }
    } else { /* equal variants */
        switch (ttypetag(t1)) {
 ...
 ** (External strings can be empty too; they will be concatenated like non-empty ones.)
 */
...
/*
 ** Integer division; return 'm // n', that is, floor(m/n). C division truncates its result (rounds towards zero).
 ** 'floor(q) == trunc(q)' when 'q >= 0' or when 'q' is integer, otherwise 'floor(q) == trunc(q) -
 ** 1'.
 */
...
/*
 ** Integer modulus; return 'm % n'. (Assume that C '%' with negative operands follows C99 behavior. See previous
 ** comment about luaV_idiv.)
 */
...
void luaV_finishOp(lua_State *L) {
    ...
    case OP_RETURN: { /* yielded closing variables */
        ...
        /* adjust top to signal correct number of returns, in case the return is "up to top"
         * ('isIT') */
    ...
    default: {
        ...
        lua_assert(
            op == OP_TFORCALL || op == OP_CALL || op == OP_TAILCALL || op == OP_SETTABUP || op == OP_SETTABLE ||
 ...
 **
 ** All these macros are to be used exclusively inside the main iterpreter loop (function luaV_execute) and may access
 ** directly the local variables of that function (L, i, pc, ci, etc.).
 ** ===================================================================
 */
...
/*
 ** Auxiliary function for arithmetic operations over floats and others with two operands.
 */
...
/*
 ** Order operations with register operands. 'opn' actually works for all numbers, but the fast track improves
 ** performance for integers.
 */
...
/*
 ** Order operations with immediate operand. (Immediate operand is always small enough to have an exact representation
 ** as a float.)
 */
...
/*
 ** Execute a jump instruction. The 'updatetrap' allows signals to stop tight loops. (Without it, the local copy of
 ** 'trap' could never change.)
 */
...
/*
 ** do a conditional jump: skip next instruction if 'cond' is not what was expected (parameter 'k'), else do next
 ** instruction, which must be a jump.
 */
#define docondjump()                                                                                                   \
    if (cond != GETARG_k(i))                                                                                           \
        pc++;                                                                                                          \
...
/*
 ** macro executed during Lua functions at points where the function can yield.
 */
...
#include "lopnames.h"
        ...
        vmdispatch(GET_OPCODE(i)) {
            ...
            vmcase(OP_TFORPREP) {
                 ...
                 * 'ra + 3' has the closing variable. This opcode then swaps the control and the closing variables and
                 * marks the closing variable as to-be-closed.
                 */
                StkId ra = RA(i);
            ...
            l_tforcall: {
                 ...
                 * 'ra + 2' has the closing variable, and 'ra + 3' has the control variable. The call will use the
                 * stack starting at 'ra + 3', so that it preserves the first three values, and the first return
