...
/*
 * WARNING: if you change the order of this enumeration, grep "ORDER TM" and "ORDER OP"
 */
typedef enum {
    TM_INDEX,
    TM_NEWINDEX,
...
/*
 ** Mask with 1 in all fast-access methods. A 1 in any of these bits in the flag of a (meta)table means the metatable
 ** does not have the corresponding metamethod field. (Bit 6 of the flag indicates that the table is using the dummy
 ** node; bit 7 is used for 'isrealasize'.)
 */
...
LUAI_FUNC lu_byte luaT_callTMres(lua_State *L, const TValue *f, const TValue *p1, const TValue *p2, StkId p3);
...
LUAI_FUNC void luaT_getvarargs(lua_State *L, struct CallInfo *ci, StkId where, int wanted, int vatab);
