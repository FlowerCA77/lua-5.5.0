 ...
 ** 'pre'/'pos' allow the macro to preserve a pointer into the stack across reallocations, doing the work only when
 ** needed. It also allows the running of one GC step when the stack is reallocated.
...
/*
 ** Maximum depth for nested C calls, syntactical nested non-terminals, and other features implemented through recursion
 ** in C. (Value must fit in a 16-bit unsigned integer. It must also be compatible with the size of the C stack.)
 */
...
LUAI_FUNC TStatus luaD_protectedparser(lua_State *L, ZIO *z, const char *name, const char *mode);
...
LUAI_FUNC TStatus luaD_pcall(lua_State *L, Pfunc func, void *u, ptrdiff_t oldtop, ptrdiff_t ef);
