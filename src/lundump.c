/*
 ** $Id: lundump.c $
 ** load precompiled Lua chunks See Copyright Notice in lua.h
 */
...
/*
 ** All high-level loads go through loadVector; you can change it to adapt to the endianness of the input
 */
...
/*
 ** Load a nullable string into slot 'sl' from prototype 'p'. The assignment to the slot and the barrier must be
 ** performed before any possible GC activity, to anchor the string. (Both
 ** 'loadVector' and
...
/*
 ** Load the upvalues for a function. The names must be filled first, because the filling of the other fields can raise
 ** read errors and the creation of the error message can call an emergency collection;
