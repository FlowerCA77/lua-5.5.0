/*
 ** $Id: ltm.c $
 ** Tag methods See Copyright Notice in lua.h
 */
...
/*
 ** Return the name of the type of an object. For tables and userdata with metatable, use their
 ** '__name' metafield, if present.
 */
...
lu_byte luaT_callTMres(lua_State *L, const TValue *f, const TValue *p1, const TValue *p2, StkId res) {
...
/*
 ** The use of 'p1' after 'callbinTM' is safe because, when a tag method is not found, 'callbinTM'
...
static void createvarargtab(lua_State *L, StkId f, int n) {
    ...
    /* No need to anchor the key: Due to the resize, the next operation cannot trigger a garbage collection */
 ...
 **                 ^ ci->func                    ^ L->top final stack: func nil ... nil extra1 ...
 ** func arg1 ... argn
 **                                          ^ ci->func
 */
static void buildhiddenargs(lua_State *L, CallInfo *ci, const Proto *p, int totalargs, int nfixparams, int nextra) {
    int i;
...
/*
 ** Get the number of extra arguments in a vararg function. If vararg table has been optimized away, that number is in
 ** the call info. Otherwise, get the field 'n' from the vararg table and check that it has a proper value (non-negative
 ** integer not larger than the stack limit).
 */
...
/*
 ** Get 'wanted' vararg arguments and put them in 'where'. 'vatab' is the register of the vararg table or -1 if there is
 ** no vararg table.
 */
